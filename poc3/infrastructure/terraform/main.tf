...
resource "aws_security_group" "teamcity" {
  name_prefix = "${var.project_name}-teamcity-"
  vpc_id      = aws_vpc.main.id
  ...
  tags = {
    Name = "${var.project_name}-teamcity-sg"
  }
}
...
resource "aws_ecr_repository" "app_repos" {
  for_each = toset(["nodejs-hello-world", "python-hello-world", "java-hello-world", "dotnet-hello-world"])
...
# TeamCity EC2 Instance
resource "aws_key_pair" "teamcity" {
  key_name   = "${var.project_name}-teamcity-key"
...
resource "aws_instance" "teamcity" {
  ...
  tags = {
    Name        = "${var.project_name}-teamcity-server"
    Environment = var.environment
  }
}
...
output "teamcity_public_ip" {
  description = "Public IP of TeamCity server"
  value       = aws_instance.teamcity.public_ip
}

output "teamcity_url" {
  description = "TeamCity server URL"
  value       = "http://${aws_instance.teamcity.public_ip}:8111"
}
